//
//  UILabel+KJAttributeTextTapAction.m
//  KJEmitterView
//
...
@interface KJAttributeTapModel : NSObject
...
@implementation KJAttributeTapModel
...
- (void)touchesBegan:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event{
    ...
    if (objc_getAssociatedObject(self, @selector(openTap))) {
        self.isTapEffect = self.openTap;
    }
    UITouch *touch = [touches anyObject];
    CGPoint point = [touch locationInView:self];
    __weak typeof(self) weakSelf = self;
    if (![self kj_getTapFrameWithTouchPoint:point result:^(NSString *string, NSRange range, NSInteger index) {
        if (weakSelf.isTapEffect) {
            [weakSelf kj_saveEffectDicWithRange:range];
            [weakSelf kj_tapEffectWithStatus:YES];
        }
    }]) {
        [super touchesBegan:touches withEvent:event];
    }
...
- (void)touchesEnded:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event{
    ...
    if (self.isTapEffect) {
        [self performSelectorOnMainThread:@selector(kj_tapEffectWithStatus:) withObject:nil waitUntilDone:NO];
    }
    UITouch *touch = [touches anyObject];
    CGPoint point = [touch locationInView:self];
    __weak typeof(self) weakSelf = self;
...
- (BOOL)kj_getTapFrameWithTouchPoint:(CGPoint)point result:(void(^)(NSString *string, NSRange range, NSInteger index))resultBlock{
    ...
    CGFloat total_height = [self kj_textSizeWithAttributedString:self.attributedText
    ...
    for (CFIndex i = 0; i < count; i++) {
        ...
        CGRect flippedRect = [self kj_getLineBounds:line point:linePoint];
        CGRect rect = CGRectApplyAffineTransform(flippedRect, transform);
        CGFloat lineOutSpace = (self.bounds.size.height - total_height) / 2;
        rect.origin.y = lineOutSpace + [self kj_getLineOrign:line];
        ...
        if (CGRectContainsPoint(rect, point)) {
            ...
            for (int k = 0; k < link_count; k++) {
                KJAttributeTapModel *model = self.attributeStrings[k];
...
- (CGRect)kj_getLineBounds:(CTLineRef)line point:(CGPoint)point{
    CGFloat ascent = 0.0f;
    ...
    height = [self kj_textSizeWithAttributedString:attributedString width:self.bounds.size.width numberOfLines:0].height;
    return CGRectMake(point.x, point.y , width, height);
}

- (CGFloat)kj_getLineOrign:(CTLineRef)line{
    CFRange range = CTLineGetStringRange(line);
    if (range.location == 0) {
        return 0.;
    }else {
        ...
        return [self kj_textSizeWithAttributedString:attributedString
                                               width:self.bounds.size.width
                                       numberOfLines:0].height;
    }
}

- (CGSize)kj_textSizeWithAttributedString:(NSAttributedString *)attributedString
                                    width:(float)width
...
/// 扩大点击域
- (void)kj_tapEffectWithStatus:(BOOL)status{
    if (self.isTapEffect) {
...
- (void)kj_saveEffectDicWithRange:(NSRange)range{
...
- (void)kj_getRangesWithStrings:(NSArray<NSString *>  *)strings{
    ...
    [strings enumerateObjectsUsingBlock:^(NSString * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        if (range.length != 0) {
            totalStr = [totalStr stringByReplacingCharactersInRange:range
                                                         withString:[weakSelf kj_getStringWithRange:range]];
            KJAttributeTapModel *model = [[KJAttributeTapModel alloc] init];
            model.range = range;
            model.string = obj;
...
- (void)kj_getRangesWithRanges:(NSArray<NSString *> *)ranges{
    if (self.attributedText == nil) {
        self.isTapAction = NO;
        return;
    }
    ...
    [ranges enumerateObjectsUsingBlock:^(NSString * obj, NSUInteger idx, BOOL * stop) {
        ...
        KJAttributeTapModel *model = [KJAttributeTapModel new];
        model.range = range;
        model.string = string;
        [weakSelf.attributeStrings addObject:model];
    }];
...
- (NSString *)kj_getStringWithRange:(NSRange)range{
...
- (KJLabelTapBlock)tapBlock{
    return objc_getAssociatedObject(self, _cmd);
}
- (void)setTapBlock:(KJLabelTapBlock)tapBlock{
...
- (void)setOpenTap:(BOOL)openTap{
    objc_setAssociatedObject(self, @selector(openTap), @(openTap), OBJC_ASSOCIATION_ASSIGN);
    self.isTapEffect = openTap;
}
...
- (void)setTapHighlightColor:(UIColor *)tapHighlightColor{
    objc_setAssociatedObject(self, @selector(tapHighlightColor), tapHighlightColor, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}
